...
const OrdersTable = ({ resultsPerPage, filter }) => {
  ...
  useEffect(() => {
    ...
    if (filter === "un-paid") {
      setData(
        response
          .filter((order) => order.status === "Chưa thanh toán")
          .slice((page - 1) * resultsPerPage, page * resultsPerPage)
      );
    }
    if (filter === "completed") {
          ...
          <TableBody>
            {data.map((user, i) => (
              <TableRow key={i}>
                ...
                <TableCell>
                  <span className="text-sm">{user.amount} VNĐ</span>
                </TableCell>
                <TableCell>
                  <Badge
                    type={
                      user.status === "Chưa thanh toán"
                        ? "danger"
                        : user.status === "Đã thanh toán"
                        ? "success"
                        : user.status === "Hoàn thành"
                        ? "warning"
                        : "neutral"
                    }
                  >
